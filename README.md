...
- 「固定像素扩边」保留原图像素，四周各增加指定的整数像素。例如 1600 × 900 原图每边扩展 100 px，输出为 1800 × 1100。单图在「背景 → 画布尺寸」中设置，批量在「画布与位置」中设置；可保存到预设。有外框时，从外框边缘向外扩展。
- 使用统一尺寸或原图比例的画布时，支持九种位置和图片大小调整，不拉伸、不裁剪原图。
- 「原图比例」按每张上传图片的宽高比生成画布，留白放在画布内。单图编辑的画布尺寸中也可一键使用选中图片的原始比例；未选中图片时使用第一张。
- 比例或自定义尺寸默认使用「高清（自动）」：画布尺寸决定构图，实际导出像素按每张原图匹配，避免高分辨率图片被缩成预设尺寸。预览顶部显示实际输出像素；需要所有成品严格同尺寸时，选择 1x、2x 或 3x。固定像素扩边始终按 1:1 导出，确保扩边宽度准确。
