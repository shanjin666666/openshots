...
/** A style preset owns the canvas dimensions, independently of the previous batch mode. */
export function batchSettingsFromPreset(preset: Omit<CanvasPreset, "id">, current: BatchSettings): Partial<BatchSettings> {
  ...
  const borderLimit = Math.min(width - padding * 2, height - padding * 2) / 8;
  return {
    sizeMode: "fixed", width, height, padding, position: "center", imageScale: 100,
